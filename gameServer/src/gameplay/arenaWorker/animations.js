/**
 * @typedef {{stop:()=>void}} Animation
 */

/**
 * @type {Map<number,Animation>}
 */
export let running = new Map();
...
const get_new_id = () => {
    ...
    if(running.has(next_id)){
        if(running.size < MAX_ANIMATIONS){
            for(id = (next_id+1)%MAX_ANIMATIONS; running.has(id) && id !== next_id; id=(id+1)%MAX_ANIMATIONS){}
            if(!running.has(id)){
        ...
        current_ref.v = setInterval(()=>{
            if(radius<4){
                ...
                current_ref.v = setTimeout(()=>{
                    ...
                    radius-=1;
                    fillRect({ min: {x: x-radius, y: y-radius}, max: {x: x+radius+1, y: y+radius+1} }, 0);
